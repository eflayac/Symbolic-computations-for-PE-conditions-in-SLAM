...
%%%%%%%%%%%%%%%%%%%%%%%% Optical flow Observability Grammian lower bound for a circular trajectory around any point %%%%%%%%%%%%%%%%%%%%%%%%
...
v_per=r_c*omega*[-cos(omega*(s-t_0)+phi_0);-sin(omega*(s-t_0)+phi_0)];
q=[(x_l(1))^2-(x_l(2))^2;2*(x_l(1))*(x_l(2))];

H=[q'*v_per;q'*v]; %Simplified gradient of the observations
H=combine(H,'sincos');
...
% savefile='Grammian_optical_flow_circle_any_centre.mat';
% save(savefile);
